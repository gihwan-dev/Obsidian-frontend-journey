...
`gitlab-slash-command`를 LangGraph HIL durable execution과 클러스터 내부 PostgreSQL 기반으로 전환해 상태 보존형 slash command 실행 토대를 마련했다.
...
- automation 라벨 재적용 시 triage 재시작 흐름과 MR 파이프라인 검토 기준까지 정리해 장기 실행 자동화 제어를 더 예측 가능하게 다듬었습니다.
