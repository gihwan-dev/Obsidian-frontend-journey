...
| 항목 | 내용 |
|------|------|
| 아키텍처 결정 | `compare-screenshots.ts`는 동기 fs API (단일 파일 비교), `capture-screenshot.ts`는 async (Playwright/네트워크 의존) |
| 다음 페이즈 영향 | Story ID 형식은 Storybook 표준 (`shared-card--common`) → Story title → ID 변환 로직 필요 |
| 다음 페이즈 영향 | 기본 threshold `0.05` (5%), `--threshold` 옵션으로 조정 가능 |
| 다음 페이즈 영향 | `#storybook-root > *` 첫 번째 자식 캡처 → Story는 단일 루트 래퍼 사용 필요 |

### Phase 2 완료 (2026-01-28)

| 항목 | 내용 |
|------|------|
| 아키텍처 결정 | SKILL에 `argument-hint` frontmatter와 `argument: $1` 패턴 적용 |
| 아키텍처 결정 | story-generator 3가지 패턴: Simple / MSW-dependent / Provider-dependent |
| 아키텍처 결정 | Story ID 변환은 Storybook 표준 규칙 따름 |
| 발견된 이슈 | `.claude/skills/` 디렉토리가 `.gitignore`에 포함 → 필요시 수정 필요 |

### Phase 3 완료 (2026-01-28)

| 항목 | 내용 |
|------|------|
| 아키텍처 결정 | 두 SKILL 로직을 단일 SKILL 내 7단계 워크플로우로 내장 (오케스트레이션 안정성) |
| 아키텍처 결정 | MCP Figma Desktop 도구 3종: `get_screenshot`, `get_design_context`, `get_variable_defs` |
| 아키텍처 결정 | 새 Story 생성 시 `--rebuild` 플래그 자동 적용 |
| 아키텍처 결정 | 정성 비교 severity 4단계: Critical / Major / Minor / Nitpick |
| 다음 영향 | 전체 완료. 실제 Figma URL + 컴포넌트로 E2E 검증 필요 |
