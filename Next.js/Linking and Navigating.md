...
dynamic segments로 링킹할 때 템플릿 리터럴과 보간을 사용해 링크의 리스트를 발생시킬 수 있다. 예를들어 블로그 포스트의 리스트를 발생시키는 방법은 다음과 같다:
...
Server Actions는 path(`revalidatePath` ) 또는 cache tag(`revalidateTag` ) 를 사용해 요구중인 data를 재검증 할 수 있다.
...
앞으로가기 또는 뒤로가기의 네비게이션은 soft navigation 동작을 수행한다. 이것은 클라이언트 사이트 캐쉬가 재사용되고 즉시 이동된다는 의미다.
